...
const TicketCheckIn = () => {
  ...
  const qrcodeScannerRef = { current: null };
  ...
  const validateTicketQR = useCallback(async (qrCodeData) => {
    try {
      ...
      // Send the QR data to the API - the backend now handles different formats
      ...
      if (apiResponse.data.data.ticket && apiResponse.data.data.ticket.event_id) {
  ...
  useEffect(() => {
    if (isScanning && !scannerInitialized) {
      ...
      // Callback for errors
      const onScanFailure = () => {
        // Do nothing on failure - this is normal when nothing is being scanned
      };
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
      ...
      if (manualInput.startsWith('data:image')) {
        setError('You pasted a QR code image. Please enter the ticket ID number instead.');
        return;
      }
  ...
  const handleConfirmCheckIn = async () => {
    ...
    try {
      ...
      // Store the response but we don't need to use it directly
      ...
      setError(checkInError.response?.data?.error || 'Failed to check in ticket');
    } finally {
      setCheckingIn(false);
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 p-4 rounded-md">
              <div className="text-blue-600 font-bold text-xl">{eventStats.validTickets}</div>
              <div className="text-gray-600">Total Tickets</div>
            </div>
            <div className="bg-green-50 p-4 rounded-md">
              <div className="text-green-600 font-bold text-xl">{eventStats.checkedInCount}</div>
            ...
            <div className="bg-yellow-50 p-4 rounded-md">
              <div className="text-yellow-600 font-bold text-xl">{eventStats.remaining}</div>
                  ...
                  <tbody>
                    {recentCheckIns.map((checkIn, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="p-2">{new Date(checkIn.checked_in_at).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <>
              ...
              {isScanning ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 bg-gray-50 rounded-md border border-dashed border-gray-300">
                  ...
                  <p className="text-gray-500">Click "Start Scanner" to activate the QR code reader</p>
                </div>
              )}
              
              {/* Manual Ticket Entry */}
              <div className="mt-4">
                <h3 className="text-md font-medium mb-2">Or Enter Ticket ID Manually</h3>
                <form onSubmit={handleManualSubmit} className="flex">
                  ...
                  <button
                    ...
                    disabled={loading}
                  >
                    {loading ? <FaSpinner className="animate-spin" /> : 'Validate'}
                  </button>
                </form>
              </div>
            </>
              ...
              <div className={`p-4 rounded-md mb-4 ${
                ...
                <div className="flex items-center">
                  ...
                  <span className="font-medium">
                    {ticketResult.status === 'VALID_TODAY' && 'Valid ticket for today\'s event'}
                    {ticketResult.status === 'FUTURE_EVENT' && 'Valid ticket for a future event'}
                    {ticketResult.status === 'PAST_EVENT' && 'Ticket is for a past event'}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{ticketResult.ticket.user_name}</div>
                    <div className="text-sm text-gray-500">{ticketResult.ticket.user_email}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{formatDate(ticketResult.ticket.event_date)}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{formatTime(ticketResult.ticket.event_time)}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{ticketResult.ticket.venue}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{ticketResult.ticket.ticket_type_name}</div>
                  </div>
                </div>
                
                <div className="pt-4 mt-2 border-t border-gray-200">
                  <div className="text-sm text-gray-500 mb-2">Check-in Status</div>
                  {ticketResult.ticket.checked_in ? (
                      ...
                      <div>
                        ...
                        <div className="text-sm">
                          {ticketResult.ticket.checked_in_at ? 
                            `at ${new Date(ticketResult.ticket.checked_in_at).toLocaleString()}` : 
                            ''}
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      disabled={checkingIn || ticketResult.status !== 'VALID_TODAY'}
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          {!ticketResult ? (
            ...
            <>
              <h2 className="text-lg font-medium mb-4">Check-In Instructions</h2>
              
              <div className="space-y-6">
                <div className="flex">
                  <div className="mr-4 bg-purple-100 rounded-full h-8 w-8 flex items-center justify-center text-purple-600 font-bold">1</div>
                  <div>
                    <div className="font-medium mb-1">Start the Scanner</div>
                    <p className="text-gray-600 text-sm">
                      Click the "Start Scanner" button to activate your device's camera. You may need to grant camera permissions.
                    </p>
                  </div>
                </div>
                
                <div className="flex">
                  <div className="mr-4 bg-purple-100 rounded-full h-8 w-8 flex items-center justify-center text-purple-600 font-bold">2</div>
                  <div>
                    <div className="font-medium mb-1">Scan Ticket QR Code</div>
                    <p className="text-gray-600 text-sm">
                      Hold the visitor's QR code in front of your camera. The system will automatically detect valid tickets.
                    </p>
                  </div>
                </div>
                
                <div className="flex">
                  <div className="mr-4 bg-purple-100 rounded-full h-8 w-8 flex items-center justify-center text-purple-600 font-bold">3</div>
                  <div>
                    <div className="font-medium mb-1">Verify Ticket Details</div>
                    <p className="text-gray-600 text-sm">
                      Check that the ticket is valid for today's event and has not been used before.
                    </p>
                  </div>
                </div>
                
                <div className="flex">
                  <div className="mr-4 bg-purple-100 rounded-full h-8 w-8 flex items-center justify-center text-purple-600 font-bold">4</div>
                  <div>
                    <div className="font-medium mb-1">Confirm Check-In</div>
                    <p className="text-gray-600 text-sm">
                      Click the "Confirm Check-In" button to mark the ticket as used. The attendee can now enter the event.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <>
              ...
              {recentCheckIns && recentCheckIns.length > 0 ? (
                    ...
                    <tbody>
                      {recentCheckIns.map((checkIn, index) => (
                        <tr key={index} className="border-b">
                          <td className="p-2">{checkIn.user_name}</td>
                          <td className="p-2">{checkIn.ticket_type}</td>
                          <td className="p-2 text-gray-500">{new Date(checkIn.checked_in_at).toLocaleTimeString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="bg-gray-50 p-6 text-center rounded-md">
                  ...
                  <p className="text-gray-500">No check-ins recorded for this event yet.</p>
                </div>
              )}
              ...
              <div className="mt-8 p-4 bg-blue-50 rounded-md border border-blue-200">
                <div className="flex items-start">
                  ...
                  <div>
                    <div className="font-medium text-blue-800 mb-1">Event Statistics</div>
                    <p className="text-sm text-blue-700">
                      {eventStats ? (
                        <>
                          {eventStats.checkedInCount} of {eventStats.validTickets} attendees checked in 
                          ({eventStats.checkInPercentage}%)
                        </>
                      ) : (
                        'Loading statistics...'
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-md">
            <div className="flex items-start">
              ...
              <div>
                <div className="font-medium text-gray-700 mb-1">Troubleshooting</div>
                <p className="text-sm text-gray-600">
                  If the scanner isn't working, simply enter the ticket ID number manually. You can find the ID on the ticket.
                </p>
              </div>
            </div>
          </div>
