...
const EventDetails = () => {
  ...
  const [reviews, setReviews] = useState({ items: [], count: 0, averageRating: 0 });
  ...
  const handleCheckout = () => {
    ...
    const ticketsData = Object.entries(selectedTickets).map(([typeId, quantity]) => {
      ...
      >
        <div className="container mx-auto px-4 h-full flex flex-col justify-end pb-16">
          <div className="text-white mb-4">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold mb-3">{event.name}</h1>
          ...
          <div className="flex items-center">
            ...
            <div className="flex space-x-3">
              <button className="bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-1 rounded-full flex items-center text-white">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-grow lg:w-2/3">
            ...
            <div className="mb-6 border-b border-gray-300">
              <div className="flex">
                <button
                  className={`px-6 py-3 font-medium ${activeTab === 'details' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-600 hover:text-gray-900'}`}
                  onClick={() => setActiveTab('details')}
                >
                  Details
                </button>
                <button
                  className={`px-6 py-3 font-medium ${activeTab === 'reviews' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-600 hover:text-gray-900'}`}
                  onClick={() => setActiveTab('reviews')}
                >
                  Reviews ({reviews.count})
                </button>
                <button
                  className={`px-6 py-3 font-medium ${activeTab === 'location' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-gray-600 hover:text-gray-900'}`}
                ...
                {event.cancellation_policy && (
                  <div className="mt-8">
                    <h3 className="text-lg font-bold mb-2 flex items-center">
                      <FaInfoCircle className="mr-2 text-purple-600" /> Cancellation Policy
                    </h3>
                    <div className="bg-purple-50 p-4 rounded-md border border-purple-100">
                      <p className="text-purple-800">{event.cancellation_policy}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <EventReviews 
                ...
                onReviewAdded={(newReview) => {
                  setReviews(prev => ({
                    ...
                    count: prev.count + 1,
                    averageRating: ((prev.averageRating * prev.count) + newReview.rating) / (prev.count + 1)
                  }));
                }}
              />
            )}
