...
const EventReviews = ({ reviews, eventId, onReviewAdded }) => {
  ...
  const userReview = user ? reviews.items.find(review => review.user_id === user.id) : null;
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      ...
      if (onReviewAdded) {
        const updatedReviews = reviews.items.filter(review => review.id !== reviewId);
        const updatedCount = reviews.count - 1;
        const updatedRating = updatedCount > 0 
          ? ((reviews.averageRating * reviews.count) - userReview.rating) / updatedCount
          : 0;
          ...
          <form onSubmit={handleSubmitReview}>
            ...
            <button
              ...
              className={`bg-purple-600 text-white px-4 py-2 rounded ${
                isSubmitting || userRating === 0 ? 'opacity-70 cursor-not-allowed' : 'hover:bg-purple-700'
              }`}
            >
              {isSubmitting ? (
                <>
