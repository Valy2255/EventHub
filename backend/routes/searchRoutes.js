...
router.get(
  ...
  WHERE e.status IN ('active', 'rescheduled')
`;
    ...
    if (startDate && endDate) {
      ...
      sqlQuery += ` AND e.date BETWEEN $${queryParams.length + 1} AND $${
    ...
    const formattedEvents = results.map((event) => ({
      ...
      // Add these fields for rescheduled events:
      status: event.status,
...
router.get(
  ...
  SELECT e.id, e.name, e.venue, e.city, e.date, e.image_url,
         e.status, e.original_date, e.original_time,
         c.name as category_name, c.slug as category_slug,
         CASE WHEN e.status = 'rescheduled' THEN true ELSE false END AS is_rescheduled
  FROM events e
  LEFT JOIN categories c ON e.category_id = c.id
...
`;
    ...
    const { rows: events } = await db.query(sqlQuery, [
      `%${query}%`,
      `${query}%`,
    ]);
